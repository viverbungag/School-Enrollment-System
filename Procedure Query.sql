...
DELIMITER $$
CREATE PROCEDURE check_time(IN sched VARCHAR(21), IN id INT, OUT checkIfValid BOOLEAN, OUT subCode INT)
BEGIN
    ...
    WHILE x < count DO
        ...
        IF day_input = day_sched AND ((time_input_begin BETWEEN time_sched_begin AND time_sched_end) OR (time_input_end BETWEEN time_sched_begin AND time_sched_end)) THEN
